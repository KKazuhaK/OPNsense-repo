...
fragments and harmless command stubs. It checks candidate validation before
live publication, root-only staging, stable no-follow reads, durable exact
rollback, resolver identity and recovery, conservative ownership receipts, and
preservation of unrelated includes. Actual Jinja2 rendering preserves enabled
directives verbatim and emits no disabled directives. PHP, when present,
exercises the real controllers' request-method, read-only and apply-result
handling with isolated framework stubs.
