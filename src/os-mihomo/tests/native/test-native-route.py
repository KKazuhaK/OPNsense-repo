...
def main(argv=None):
    ...
    parser.add_argument('--helper', default='/usr/local/opnsense/scripts/mihomo/native_route.py')
    parser.add_argument('--routing', default='/usr/local/opnsense/scripts/mihomo/routing.py')
    ...
    def routes(fib, family=4):
        flag = 'inet' if family == 4 else 'inet6'
        raw = command(['/usr/bin/netstat', '-rn', '-F', str(fib), '-f', flag]).stdout.decode()
        return routing.parse_routes(raw, family)
