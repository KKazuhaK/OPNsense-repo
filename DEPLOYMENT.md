...
The harness requires root, VIMAGE, TUN and enough disk for its disposable filesystem. It creates an isolated VNET with synthetic upstreams and its own FIBs. It does not connect the jail to production interfaces. Its report identifies the exact package SHA-256 and executed checks. Run it a second time with `MIHOMO_JAIL_DNSSEC=1` to repeat the lifecycle against an Unbound configuration that validates DNSSEC, which the DNS integration must leave unchanged; that round writes `test-report-dnssec.json` beside the package, so the report used for signing stays the base round's. Test adapters replace OPNsense configd/filter/template/GUI infrastructure; actual package hooks, core, daemon, TUN, routes and Unbound are exercised. Remaining production checks are listed below.
...
A version that is already published may never be republished with different content. Earlier archives stay downloadable from `All/` for rollback, but each signed catalog offers only the newest version of every package, compared numerically (1.2.10 after 1.2.9). Offered several versions of one package, pkg 2.3 does not reliably install the newest (the published catalog once served 1.2.9 over 1.2.10 to fresh installs and to upgrades from this repository); the verifier refuses a catalog that offers a package more than once or not at its newest published version. The signed `release.json` records the digest of every superseded archive and the verifier checks each one, so nothing in `All/` goes unsigned. Roll back with `pkg add` of the archive URL after comparing its SHA-256 with that record, not `pkg install name-version`. `python3 verify-repo.py .site --audit --source .` lists every recorded plugin as unpublished, unchanged, rejected, or changed without a version bump, and exits non-zero on the last of those. `build-repo.sh` runs it over the published tree before preparing a release, so it refuses to sign while any published version no longer matches its committed source.
