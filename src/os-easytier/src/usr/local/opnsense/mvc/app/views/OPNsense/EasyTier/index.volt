...
<div class="tab-content et-content">
  <div class="tab-pane active" id="et-status">
    <form id="frm_easytier_status"><div class="content-box"><table class="table table-condensed table-striped et-summary">
      ...
      <tr><th data-et-label="Imported VPN routes">{{ lang._('Imported VPN routes') }}</th><td id="et-imported-routes">-</td></tr>
      <tr><th data-et-label="Skipped public or invalid routes">{{ lang._('Skipped public or invalid routes') }}</th><td id="et-public-routes">-</td></tr>
      <tr><th data-et-label="Skipped routes conflicting with native networks">{{ lang._('Skipped routes conflicting with native networks') }}</th><td id="et-native-routes">-</td></tr>
      <tr><th data-et-label="Network supervision">{{ lang._('Network supervision') }}</th><td id="et-network-error">-</td></tr>
    ...
    <div class="alert alert-info" data-et-label="VPN peer ingress needs administrator firewall rules. In Firewall > Rules > Floating, use Direction in, Interface any, and restrict VPN sources, local destinations and required ports. Existing pass/block rules control access.">{{ lang._('VPN peer ingress needs administrator firewall rules. In Firewall > Rules > Floating, use Direction in, Interface any, and restrict VPN sources, local destinations and required ports. Existing pass/block rules control access.') }}</div>
  </div>
...
$(function () {
    ...
    function refreshStatus() {
        ajaxGet('/api/easytier/service/status', {}, function (data) {
            ...
            $('#et-imported-routes').text(data.imported_routes || 0);
            $('#et-public-routes').text((data.rejected_routes || {}).public_or_invalid || 0);
            $('#et-native-routes').text((data.rejected_routes || {}).native_conflict || 0);
            $('#et-network-error').text(htmlDecode(data.network_error || '-'));
