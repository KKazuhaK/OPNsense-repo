...
$failed = 0;
$root = getenv('STATICARP_ROOT') ?: '';
foreach ([
    $root . '/usr/local/opnsense/scripts/staticarp/settings.php' => ['util.inc', 'config.inc', 'interfaces.inc'],
] as $file => $includes) {
    if (!is_readable($file)) { printf("  X %-24s not found\n", basename($file)); $failed++; continue; }
    $pid = pcntl_fork();
    if ($pid === 0) {
