...
gVisor 协议栈每条 TCP 连接最多只有 20 KB 在途，单条连接的速度约为每个往返 20 KB：即使规则为 `DIRECT`，Wi-Fi 客户端也只有几十 Mbps。“快速 TCP 路径”（默认关闭）改用 PF 重定向，把选中来源的新 IPv4 TCP 交给核心在 `127.0.0.1:7894` 上的监听器，由内核 TCP 协议栈承载；测试环境中 18 ms 往返的单条连接从 TUN 的 8 Mbps 提升到约 1 Gbps。核心仍能取得原始目的地址，规则、嗅探和节点选择不变；连接的入站显示为 `opnsense-transparent-tcp` 而不是 TUN。UDP（含 QUIC）、到 53 端口的 TCP（供 DNS 劫持）和 IPv6 仍走 TUN。只有正在运行的核心确实占有该监听器、且防火墙已加载插件的 `rdr-anchor` 时才启用重定向，否则 TCP 继续走 TUN，状态会显示原因。停止服务或核心退出时会撤销重定向并清除相关连接。防火墙规则看到的被重定向 TCP 目的地址是 `127.0.0.1` 的 7894 端口：按目的主机或端口阻止的规则、阻止访问防火墙本身的规则，以及指定网关的 LAN 规则，对被接管的 TCP 不再生效。启用前请检查被接管接口上的规则。
