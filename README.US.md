...
Legacy FreeBSD 14/15 downloads and the `os-mihomo` 1.0.2 rollback package remain available. Current maintenance targets FreeBSD 15. Since 1.0.3, `os-sing-box` has removed third-party subscription conversion and accepted complete JSON only. The current Ttyd build uses pinned FreeBSD 15 assets; retained FreeBSD 14 assets are excluded from current target builds. Legacy packages retained from [Opnwall/OPNsense-repo](https://github.com/Opnwall/OPNsense-repo) are compatibility downloads, without current test certification.
