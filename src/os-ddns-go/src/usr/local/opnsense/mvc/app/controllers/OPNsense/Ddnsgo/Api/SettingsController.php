...
class SettingsController extends ApiControllerBase
{
    ...
    {
        $decoded = json_decode((string)(new Backend())->configdRun('ddnsgo get'), true);
    ...
    public function setAction(): array
    {
        ...
        try {
            ...
            $decoded = json_decode((string)(new Backend())->configdpRun('ddnsgo set', [$staged]), true);
        } finally {
            @unlink($staged);
        }
        if (!is_array($decoded)) {
        ...
        if (($decoded['status'] ?? '') === 'ok') {
            $action = 'restart';
            $output = (string)(new Backend())->configdRun('ddnsgo ' . $action);
            if (trim($output) !== 'OK') {
