...
sh src/os-mihomo/tests/jail/run.sh \
  src/os-mihomo/dist/FreeBSD:15:amd64/os-mihomo-1.2.0.pkg legacy-1.0.2.pkg
```
...
SOURCE_COMMIT=<tested-40-character-commit> \
  ...
  sh build-repo.sh src/os-mihomo/dist/FreeBSD:15:amd64/os-mihomo-1.2.0.pkg \
...
7. Before production use of multi-WAN DNAT with an `auto-route` TUN, verify that inbound PF states bind replies to the correct WAN gateway using `reply-to`. Inline NAT **Pass** skips subsequent filter rules; **Register rule** needs the correct gateway and `reply-to` enabled globally and per rule. Confirm a stateful WAN rule with the required return binding rather than relying on LAN source-device policy. For an example remote client `198.51.100.10` and DNAT target `192.0.2.10`, inspect `route -n get 198.51.100.10`, `pfctl -sr` and `pfctl -ss -vv`, and capture SYN/SYN-ACK on WAN2, LAN and `tun_mihomo`. Use new connections after rule changes; if needed, remove only affected old states, never the complete state table. NAT remains administrator-maintained, and 1.2.2 does not rewrite or fix this policy. See [NAT association](https://docs.opnsense.org/manual/nat.html#filter-rule-association), [WAN reply routing](https://docs.opnsense.org/manual/firewall.html) and the [Mihomo configuration notes](src/os-mihomo/README.US.md).
