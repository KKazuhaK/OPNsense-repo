...
$(function () {
    ...
    function message(value, success) {$('#et-message').removeClass('hidden alert-success alert-danger').addClass(success ? 'alert-success' : 'alert-danger').text(htmlDecode(t(value)));}
    function mutate(action, done) {
        ...
        ajaxCall('/api/easytier/service/' + action, {}, function (data) {
            ...
            else {message('Service command submitted.', true);}
    ...
    $('.et-save').click(function () {
        ...
        ajaxCall('/api/easytier/settings/set', {config: $('#et-config').val()}, function (data) {
            ...
            message('Configuration saved.', true); loadSettings(); if (restart) {mutate('restart');}
        });
    });
