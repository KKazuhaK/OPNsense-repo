...
gVisor 协议栈每条 TCP 连接最多只有 20 KB 在途，单条连接的速度约为每个往返 20 KB：即使规则为 `DIRECT`，Wi-Fi 客户端也只有几十 Mbps。“快速 TCP 路径”（默认关闭）改用 PF 重定向，把选中来源的新 IPv4 TCP 交给核心在 `127.0.0.1:7894` 上的监听器，由内核 TCP 协议栈承载；测试环境中 18 ms 往返的单条连接从 TUN 的 8 Mbps 提升到约 1 Gbps。核心仍能取得原始目的地址，规则、嗅探和节点选择不变；连接的入站显示为 `opnsense-transparent-tcp` 而不是 TUN。UDP（含 QUIC）、到 53 端口的 TCP（供 DNS 劫持）和 IPv6 仍走 TUN。只有正在运行的核心确实占有该监听器、且防火墙已加载插件的 `rdr-anchor` 时才启用重定向，否则 TCP 继续走 TUN，状态会显示原因。停止服务或核心退出时会撤销重定向并清除相关连接。防火墙规则看到的被重定向 TCP 目的地址是 `127.0.0.1` 的 7894 端口。按目的主机或端口阻止的规则对被接管的 TCP 不再生效；只放行特定目的主机或端口的规则不再匹配，流量会落入默认阻止；阻止访问防火墙本身的规则会将其阻止。指定网关的 LAN 规则仍会匹配，并把被重定向的连接路由到该网关，连接因此丢失。启用快速路径前，请在这类规则之上放一条不指定网关、放行被接管来源到 `127.0.0.1` 7894 端口 TCP 的规则，或把这些来源排除在接管之外。

设备策略在流量进入 TUN 前按 LAN 源地址选择 TCP/UDP：黑名单中的地址绕过 TUN，白名单只有列出的地址进入 TUN；关闭设备筛选或列表为空时，所有符合接管条件的 LAN 源的 TCP/UDP 进入 TUN。ICMP 及其他控制流量沿用普通路由。绕过的设备沿用现有路由，不经过 Mihomo 的 `DIRECT` 转发。进入 TUN 的流量再遵循订阅和合并规则。用户已有防火墙允许/阻止规则及路由策略仍生效；路由器自身流量和 WAN 入站流量不接管。正常有状态防火墙/NAT 规则下，已建立 DNAT 连接的回包沿用既有状态和正确配置的回程路由。手动配置 HTTP/SOCKS 代理的客户端不受此设备筛选限制。“接管接口”把透明代理限定在选中的内侧接口；留空与旧版本一致，接管所有内侧接口。WAN 类接口永远不会被接管，也不会出现在选项里：接口名为 WAN，或 OPNsense 为它解析出网关，都算 WAN 类，覆盖多 WAN 的每条上行、PPPoE 和带网关的 VPN 出口。网桥请选网桥接口而不是成员口；VPN 服务器需先把隧道分配为接口才能选择。设备策略在选中的接口范围内生效，访问任何本地网络的流量仍然绕过 TUN。
